...
n=as.integer(readline("number of employees"))
EmpID<-as.vector(n)
EmpName<-as.vector(n)
DOJ<-as.vector(n)
EmpCode<-as.vector(n)
...
y = data.frame("24", "Jaff", "16/09/28", "emp3", "net", "CRO")
write.table(y, file="l4.csv", append = TRUE, sep = ",", quote = FALSE, col.names = FALSE, row.names = FALSE)
read.csv("l4.csv")
